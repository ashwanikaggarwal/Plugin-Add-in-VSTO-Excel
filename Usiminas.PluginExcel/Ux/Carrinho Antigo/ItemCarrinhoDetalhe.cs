...
namespace Usiminas.PluginExcel.Ux
{
    public partial class ItemCarrinhoDetalhe : UserControl
    {
        ...
        {
            string mes = OvDtAceiteDisp.Value.Month.ToString().Length == 1 ? "0" + OvDtAceiteDisp.Value.Month.ToString() : OvDtAceiteDisp.Value.Month.ToString();
            if (OvDtAceiteDisp.Value.Day >= 1 && OvDtAceiteDisp.Value.Day <= 10)
            {
                OvDtAceiteDisp.Value = DateTime.ParseExact("01/" + mes + "/" + OvDtAceiteDisp.Value.Year, "dd/MM/yyyy", CultureInfo.InvariantCulture);
                DispDecendio = "D1";

            }
            else if (OvDtAceiteDisp.Value.Day >= 11 && OvDtAceiteDisp.Value.Day <= 20)
            {
                OvDtAceiteDisp.Value = DateTime.ParseExact("11/" + mes + "/" + OvDtAceiteDisp.Value.Year, "dd/MM/yyyy", CultureInfo.InvariantCulture);
                DispDecendio = "D2";

            }
            else if (OvDtAceiteDisp.Value.Day >= 21 && OvDtAceiteDisp.Value.Day <= 31)
            {
                OvDtAceiteDisp.Value = DateTime.ParseExact("21/" + mes + "/" + OvDtAceiteDisp.Value.Year, "dd/MM/yyyy", CultureInfo.InvariantCulture);
                DispDecendio = "D3";
            }
